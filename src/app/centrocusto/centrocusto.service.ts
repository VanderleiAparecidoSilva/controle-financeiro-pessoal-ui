...
export class CentrocustoService {
  ...
  findAllActive(): Promise<any> {
    const httpOptions = {
      params: new HttpParams()
        .set('email', this.auth.jwtPayload.user_name)};
    return this.httpClient.get<any>(`${environment.apiUrl}${api_dominio.centroCusto}/ativos`, httpOptions)
      .toPromise()
      .then( response => {
        return response;
      });
  ...
  findAllActiveByType(tipo: any): Promise<any> {
    const httpOptions = {
      params: new HttpParams()
        .set('email', this.auth.jwtPayload.user_name)
        .set('tipo', tipo)};
    return this.httpClient.get<any>(`${environment.apiUrl}${api_dominio.centroCusto}/ativos/tipo`, httpOptions)
      .toPromise()
      .then( response => {
        return response;
      });
